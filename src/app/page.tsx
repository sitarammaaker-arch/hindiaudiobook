...
export default async function HomePage() {
  ...
  return (
    <main>
      {/* ── Hero — Saffron Orange Indian Theme ── */}
      <section className="relative overflow-hidden py-16 md:py-24"
        style={{ background: "linear-gradient(135deg, #FF6B2B 0%, #E85A1A 40%, #C94A0E 70%, #7C1D00 100%)" }}>

        {/* Decorative circles — Indian mandala feel */}
        ...
        <div className="absolute bottom-0 left-0 w-72 h-72 rounded-full opacity-10"
          style={{ background: "radial-gradient(circle, #FFA500, transparent)", transform: "translate(-30%, 30%)" }} />
        <div className="absolute top-1/2 left-1/2 w-[600px] h-[600px] rounded-full opacity-5"
          style={{ background: "radial-gradient(circle, #FFD700, transparent)", transform: "translate(-50%, -50%)" }} />

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative">
          {/* Badge */}
          <div className="inline-flex items-center gap-2 bg-white/15 border border-white/30 text-white text-xs font-semibold px-4 py-2 rounded-full mb-6 backdrop-blur-sm">
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-white mb-4 leading-tight drop-shadow-lg"
            style={{ fontFamily: "var(--font-merriweather)", textShadow: "0 2px 20px rgba(0,0,0,0.3)" }}>
            Free Hindi Audiobooks<br />
            <span style={{ color: "#FFD700" }}>Bilkul Free — Online Sunein</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-3 justify-center mb-10">
            <Link href="/search"
              className="font-bold px-8 py-3.5 rounded-xl transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5"
              style={{ background: "#FFD700", color: "#7C1D00" }}>
              🔍 Search Karein
            </Link>
            <Link href="/free-hindi-audiobooks"
              className="bg-white/15 hover:bg-white/25 border border-white/40 text-white font-bold px-8 py-3.5 rounded-xl transition-all backdrop-blur-sm">
            ...
            <a href="https://whatsapp.com/channel/0029VaFSHSB1dAw9L0JBGn0P" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 font-semibold px-6 py-3.5 rounded-xl transition-all text-sm backdrop-blur-sm"
              style={{ background: "#25D36630", border: "1px solid #25D36660", color: "#ffffff" }}>
          ...
          <div className="max-w-md mx-auto">
            <p className="text-white/80 text-sm mb-3">
              🔔 Nayi audiobooks ki notification ke liye subscribe karein — bilkul free
            </p>
